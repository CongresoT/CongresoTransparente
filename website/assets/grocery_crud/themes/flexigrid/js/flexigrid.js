$(function(){
	...
	$('.tDiv2').on('click','.delete-row', function(){

		var ids = new Array();
		$('.multi-row:checked').each(function( index, element ) { ids.push($(element).attr('value'));  });
		if (ids.length == 0 )
		{
			error_message(message_delete_list);
			return false;			
		}

		var delete_url = $(this).attr('href') + '/' + ids.join('-');
		var this_container = $(this).closest('.flexigrid');
		if( confirm( message_alert_delete ) )
		{
			$.ajax({
