...
class grocery_CRUD_Model_Driver extends grocery_CRUD_Field_Types
{
	...
	protected function _get_field_names_to_search(array $relation_values)
	{
		...
		else
		{
			...
			$i = 0;
			while ($i < count($field_names_array))
			{
		...
		if($this->callback_delete === null)
		{
			if($this->callback_before_delete !== null)
			{
				...
				if (is_array($primary_key))
				{
					...
					foreach ($primary_key as $key => $value) 
						$callback_return = $callback_return && call_user_func($this->callback_before_delete, $value);
				}
				else
			...
			if($this->callback_after_delete !== null)
			{
				if (is_array($primary_key))
				{
					$callback_return = true;
					foreach ($primary_key as $key => $value) 
						$callback_return = $callback_return && call_user_func($this->callback_after_delete, $value);
				}
				else
...
class grocery_CRUD_Layout extends grocery_CRUD_Model_Driver
{
	...
	protected function showList($ajax = false, $state_info = null)
	{
		...
		$data->unset_multi			= $this->unset_multi;
		...
		foreach($data->list as $num_row => $row)
		{
			...
			$data->list[$num_row]->multi_primary_key_value = $row->{$data->primary_key};
		}
	...
	protected function get_upload_file_input($field_info, $value)
	{
		...
		if ($is_image)
			$input .= " $image_class'><img src='".$file_url."' height='50px'>";
		elseif ($is_audio) 
			$input .= " $image_class'><audio controls><source src='".$file_url."' type='audio/mp3'></audio>";
		else
	...
	public function getStateInfo()
	{
		...
		switch ($state_code) {
			...
			case 4:
				if($first_parameter !== null)
				{
					if (strpos($first_parameter, '-') > 0)
						$state_info = (object)array('primary_key' => explode('-', $first_parameter));
					else
						$state_info = (object)array('primary_key' => $first_parameter);
				}
				else
...
class Grocery_CRUD extends grocery_CRUD_States
{
	...
	/**
	 * Unsets the multi operation from the list
	 *
	 * @return	void
	 */
	public function unset_multi()
	{
		$this->unset_multi = true;
