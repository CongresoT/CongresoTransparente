...
class grocery_CRUD_Model  extends CI_Model  {
    ...
    {
        if (is_array($main_primary_key))
        {
            $this->db->where_in($field_info->primary_key_alias_to_this_table, $main_primary_key);
            $this->db->delete($field_info->relation_table);            
        }            
        else
        {
    ...
    {
		if ($this->field_exists('modification_date'))
		{
			$this->load->helper('date');
			$post_array['modification_date'] = date('Y-m-d H:i:s');
		}

		if ($this->field_exists('modification_admin_id'))
		{
			if ($this->session->is_admin())
				$post_array['modification_admin_id'] = $this->session->get_user_id();
		}
    ...
    {
		if ($this->field_exists('creation_date'))
		{
			$this->load->helper('date');
		...
		if ($this->field_exists('creation_admin_id'))
		{
			if ($this->session->is_admin())
				$post_array['creation_admin_id'] 	= $this->session->get_user_id();
		}
    ...
    function db_delete($primary_key_value)
    {
        ...
        {
            $this->db->where_in($primary_key_field, $primary_key_value);
