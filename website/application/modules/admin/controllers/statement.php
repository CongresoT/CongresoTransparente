...
/**
  ...
  *
  * This is the statement controller for the administrator console of the site.
  *
...
class Statement extends MY_Controller {
	...
	function  __construct() {		
		...
		$this->module_name	= 'statement';
		// END: Set module name
		
		// BEGIN: Set title
		$this->data_parts['title'] = 'Pronunciamientos';
	...
	public function index() {
		...
		$this->grocery_crud->set_table('statement');
		$this->grocery_crud->set_subject('Pronunciamiento');
		
		$this->grocery_crud->display_as('statement_id','ID');
		...
		$this->grocery_crud->display_as('datetime','Fecha');
		
		$this->grocery_crud->fields('congressman_id', 'subject', 'description', 'datetime');
		$this->grocery_crud->columns('congressman_id', 'subject', 'datetime');
		...
		$this->grocery_crud->set_rules('datetime','Fecha','required');
		$this->grocery_crud->required_fields('congressman_id', 'subject', 'datetime');
	...
	{
		return $this->admin_model->add_to_log($this->session->get_user_id(), 'Se ingresó el pronunciamiento ' . $post_array['subject'] . ' con ID ' . $primary_key . '.');
	}
	...
	{
		return $this->admin_model->add_to_log($this->session->get_user_id(), 'Se actualizó el pronunciamiento con ID ' . $primary_key . '.');
	}
	...
	{
		return $this->admin_model->add_to_log($this->session->get_user_id(), 'Se eliminó el pronunciamiento con ID ' . $primary_key . '.');
	}
}